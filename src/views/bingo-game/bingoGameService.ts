...
export function clearBoard(bingoCard: BingoCell[][]): BingoCell[][] {
  for (const row of bingoCard) {
    for (const cell of row) {
      if (!cell.isFreeSpace) cell.isMarked = false;
    }
  }
  return bingoCard;
}

export function addBoardToLocalStorage(bingoCard: BingoCell[][]) {
  localStorage.setItem("bingo-board", JSON.stringify(bingoCard));
}
